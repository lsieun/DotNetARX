...
namespace DotNetARX
{
    ...
    public static class PlotTools
    {
        ...
        public static PreviewEndPlotStatus Plot(this PlotEngine engine, Layout layout, PlotSettings ps, string fileName,
            int copies, bool isPreview, bool showProgressDialog, bool plotToFile)
        {
            ...
            if (showProgressDialog) //如果需要显示打印进度框，则创建
            {
                ...
                plotDlg.set_PlotMsgString(PlotMessageIndex.SheetName,
                    "正在处理图纸：" + layout.LayoutName + "(" + plotFileName + ".dwg)");
            }
        ...
        /// <param name="plotToFile">是否打印到文件</param>
        /// <returns>返回打印状态</returns>
        public static PreviewEndPlotStatus MPlot(this PlotEngine engine, List<Layout> layouts, PlotSettings ps,
            string fileName, int previewNum, int copies, bool showProgressDialog, bool plotToFile)
        {
        ...
        public static void CopyPlotSettings(this Database destdb, Database sourceDb, string plotSettingName)
        {
            using (Transaction trSource = sourceDb.TransactionManager.StartTransaction())
            {
                using (Transaction trDest = destdb.TransactionManager.StartTransaction())
                {
                    //获取源图形数据库的打印设置字典
                    DBDictionary dict =
                        trSource.GetObject(sourceDb.PlotSettingsDictionaryId, OpenMode.ForRead) as DBDictionary;
