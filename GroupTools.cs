...
namespace DotNetARX
{
    public static class GroupTools
    {
        ...
        /// <param name="ids">要加入实体的 ObjectId 集合</param>
        /// <returns>返回组的 Id</returns>
        public static ObjectId CreateGroup(this Database db, string groupName, ObjectIdList ids)
        {
            ...
            // 在组字典中加入新创建的组对象，并指定它的搜索关键字为 groupName
            ...
            return group.ObjectId; // 返回组的 Id
        }

        /// <summary>
        /// 向已有组中添加对象
        /// </summary>
        /// <param name="groupId">组对象的 Id</param>
        /// <param name="entIds">要加入到组中的实体的 ObjectId 集合</param>
        ...
        public static IEnumerable<ObjectId> GetGroups(this ObjectId entId)
        {
            ...
            if (ids != null && ids.Count > 0)
            {
                ...
                if (groupIds.Count() > 0) // 如果实体属于组
                    return groupIds; // 返回实体所在组的 ObjectId
            }
        ...
        public static void RemoveGroup(this Database db, string groupName)
        {
            // 获取数据库的组字典对象
            DBDictionary groupDict = (DBDictionary)db.GroupDictionaryId.GetObject(OpenMode.ForRead);
            // 在组字典中搜索关键字为 groupName 的组对象
            if (groupDict.Contains(groupName)) // 如果找到名为 groupName 的组
            {
