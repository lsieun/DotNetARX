...
namespace DotNetARX
{
    ...
    public static class TextTools
    {
        ...
        {
            // 打开文字样式表记录
            TextStyleTableRecord record = (TextStyleTableRecord)txt.TextStyleId.GetObject(OpenMode.ForRead);
            // 必须保证文字为写的状态
            ...
            txt.Oblique = record.ObliquingAngle; // 设置倾斜角(弧度)
            txt.Annotative = record.Annotative; // 设置文字的注释性
            // 文字方向与布局是否匹配
            txt.SetPaperOrientation(Convert.ToBoolean(record.PaperOrientation));
            txt.WidthFactor = record.XScale; // 设置宽度比例
            txt.Height = record.TextSize; // 设置高度
            if (record.FlagBits == 2)
            {
                txt.IsMirroredInX = true; // 颠倒
            }
            else if (record.FlagBits == 4)
            {
                txt.IsMirroredInY = true; // 反向
            }
            else if (record.FlagBits == 6) // 颠倒并反向
            {
