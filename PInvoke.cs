...
namespace DotNetARX
{
    public static class PInvoke
    {
        ...
        {
            get { return 4 == IntPtr.Size; }
        }

        /// <summary>
        /// 将非托管函数指针转换为委托。
        /// </summary>
        /// <param name="acdb">true 表示非托管函数为 acdb**.dll 中定义的，false 表示函数为 acad.exe 中定义的</param>
        /// <param name="funcName32">32 位函数的重整名称</param>
        /// <param name="funcName64">64 位函数的重整名称</param>
        ...
        public static Delegate GetDelegateForFunction(bool acdb, string funcName32, string funcName64, Type t)
        {
            ...
            string lpFileName = "acad.exe"; // 缺省认为函数位于 acad.exe 中
            // 根据系统的位数（32 或 64 位）设定要调用的函数名
            string functionName = Is32Bit ? funcName32 : funcName64;
            if (acdb) // 如果非托管函数为 acdb**.dll 中定义的
            {
                // 获取 acdb**.dll 的主版本号
                string dllName = Register.GetACDBDLL_NAME();
                switch (dllName) // 根据主版本号确定 acdb**.dll 的具体名称
                {
                    case "R17":
            ...
            // 装载指定的动态链接库（acad.exe 或 acdb**.dll）
        ...
        private delegate bool acdbGetPreviewBitmapFromDwg([MarshalAs(UnmanagedType.LPWStr)] string pszDwgfilename,
            ref IntPtr pPreviewBmp, ref IntPtr pRetPal);

        /// <summary>
        /// 获取 dwg 文件的预览图案
        /// </summary>
        /// <param name="db">数据库对象，无意义，只是为了创建扩展函数</param>
        /// <param name="fileName">dwg 文件名</param>
        /// <returns>返回 dwg 文件的预览图案</returns>
        public static Bitmap GetPreviewBitmapFromDwg(this Database db, string fileName)
        {
            ...
            // 32 位和 64 位 acdbGetPreviewBitmapFromDwg 函数的重整名称
            ...
            // 获取表示 acdbGetPreviewBitmapFromDwg 函数的委托
            acdbGetPreviewBitmapFromDwg delegateFunc =
                (acdbGetPreviewBitmapFromDwg)GetDelegateForFunction(true, func32, func64,
                    typeof(acdbGetPreviewBitmapFromDwg));
            // 调用委托，执行 acdbGetPreviewBitmapFromDwg 函数
        ...
        /// <param name="objId">永久反应器的 Id</param>
        public static void AddPersistentReactor(this DBObject dbObject, ObjectId objId)
        {
            ...
            addPersistentReactor delegateFunc =
                (addPersistentReactor)GetDelegateForFunction(true, func, func, typeof(addPersistentReactor));
        ...
        /// <returns>若为 true,则表示将数据库看为可缩放块</returns>
        public static bool AnnotativeDwg(this Database db)
        {
            // 32 位和 64 位 annotativeDwg 函数的重整名称
            string func32 = "?annotativeDwg@AcDbDatabase@@QBE_NXZ";
            string func64 = "?annotativeDwg@AcDbDatabase@@QEBA_NXZ";
            // 获取表示 annotativeDwg 函数的委托
            annotativeDwg delegateFunc =
                (annotativeDwg)GetDelegateForFunction(true, func32, func64, typeof(annotativeDwg));
            // 调用委托，执行 annotativeDwg 函数
