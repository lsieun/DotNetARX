...
namespace DotNetARX
{
    public static class LinqToCAD
    {
        /// <summary>
        /// 获取数据库中类型为 T 的所有实体(对象打开为读）
        /// </summary>
        /// <typeparam name="T">实体的类型</typeparam>
        /// <param name="db">数据库对象</param>
        /// <returns>返回数据库中类型为 T 的实体</returns>
        ...
        public static List<T> GetEntsInDatabase<T>(this Database db, OpenMode mode, bool openErased) where T : Entity
        {
            ...
            // 声明一个 List 类的变量，用于返回类型为 T 为的实体列表
            List<T> ents = new List<T>();
            // 获取类型 T 代表的 DXF 代码名用于构建选择集过滤器
        ...
        /// <param name="openErased">是否打开已删除的实体</param>
        /// <returns>返回类型为 T 的实体</returns>
        public static List<T> SelectCrossingWindow<T>(this Database db, Point3d pt1, Point3d pt2, OpenMode mode,
            bool openErased) where T : Entity
        {
            var trans = db.TransactionManager;
            string dxfname = RXClass.GetClass(typeof(T)).DxfName;
            List<T> ents = new List<T>();
        ...
        /// <summary>
        /// 获取模型空间中类型为 T 的所有实体(对象打开为读）
        /// </summary>
        /// <typeparam name="T">实体的类型</typeparam>
        /// <param name="db">数据库对象</param>
        /// <returns>返回模型空间中类型为 T 的实体</returns>
        ...
        /// <typeparam name="T">实体的类型</typeparam>
        /// <param name="db">数据库对象</param>
        /// <returns>返回图纸空间中类型为 T 的实体</returns>
        ...
        /// <param name="openErased">是否打开已删除的实体</param>
        /// <returns>返回选择的实体</returns>
        public static List<Entity> SelectCrossingWindow(this Database db, Point3d pt1, Point3d pt2, OpenMode mode,
            bool openErased)
        {
            var trans = db.TransactionManager;
            List<Entity> ents = new List<Entity>();
