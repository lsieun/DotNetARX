...
namespace DotNetARX
{
    ...
    {
        // 更新进度条的次数，由于一些特殊原因，你必须把它设置为 600
        ...
        public void SetTotalOperations(long totalOps)
        {
            // 设置进度条需要更新的增量
            updateIncrement =
                (totalOps > progressMeterIncrements ? totalOps / progressMeterIncrements : totalOps
                );
        }
        ...
        public void Tick()
        {
            ...
            if (++currentInc == updateIncrement)
            {
                ...
                currentInc = 0; // 更新完毕，重置进度增量为 0
                // 让 AutoCAD 在长时间任务处理时仍然能接收消息
