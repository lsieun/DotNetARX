...
namespace DotNetARX
{
    ...
    public static class DimTools
    {
        ...
        public static void CreateTolerance(this FeatureControlFrame frame,
            string geometricSym, string tolerance,
            string firstDatum, string secondDatum, string thirdDatum)
        {
            if (frame == null) return; // 特征控制框对象必须已定义，否则返回
            // 设置形位公差值，各组成部分用竖线（%%v）分隔
            frame.Text = geometricSym + "%%v" + tolerance + "%%v" + firstDatum + "%%v" + secondDatum + "%%v" +
                         thirdDatum;
        }
    }
}