...
namespace DotNetARX
{
    public static class CUITools
    {
        ...
        /// 获取并打开主 CUI 文件
        /// </summary>
        /// <param name="doc">AutoCAD 文档对象</param>
        /// <returns>返回主 CUI 文件</returns>
        ...
        /// </summary>
        /// <param name="doc">AutoCAD 文档对象</param>
        /// <param name="cuiFile">CUI 文件名</param>
        /// <param name="menuGroupName">菜单组的名称</param>
        /// <returns>返回创建的 CUI 文件</returns>
        public static CustomizationSection AddCui(this Document doc, string cuiFile, string menuGroupName)
        {
            CustomizationSection cs; // 声明 CUI 文件对象
            if (!File.Exists(cuiFile)) // 如果要创建的文件不存在
            {
                cs = new CustomizationSection(); // 创建 CUI 文件对象
                cs.MenuGroupName = menuGroupName; // 指定菜单组名称
                cs.SaveAs(cuiFile); // 保存 CUI 文件
            }
            // 如果已经存在指定的 CUI 文件，则打开该文件
            else cs = new CustomizationSection(cuiFile);

            return cs; // 返回 CUI 文件对象
        }

        /// <summary>
        /// 装载指定的局部 CUI 文件
        /// </summary>
        /// <param name="cs">CUI 文件</param>
        public static void LoadCui(this CustomizationSection cs)
        {
            if (cs.IsModified) cs.Save(); // 如果 CUI 文件被修改，则保存
            // 保存 CMDECHO 及 FILEDIA 系统变量
            object oldCmdEcho = Application.GetSystemVariable("CMDECHO");
            object oldFileDia = Application.GetSystemVariable("FILEDIA");
            // 设置 CMDECHO=0，控制不在命令行上回显提示和输入信息
            Application.SetSystemVariable("CMDECHO", 0);
            // 设置 FILEDIA=0，禁止显示文件对话框，这样可以通过程序输入文件名
        ...
        /// 添加菜单项所要执行的宏
        /// </summary>
        /// <param name="source">CUI 文件</param>
        ...
        public static MenuMacro AddMacro(this CustomizationSection source, string name, string command, string tag,
            string helpString, string imagePath)
        {
            MenuGroup menuGroup = source.MenuGroup; // 获取 CUI 文件中的菜单组
        ...
        /// <param name="macroId">菜单项的命令宏的 Id</param>
        ...
        public static PopMenu AddSubMenu(this PopMenu parentMenu, int index, string name, string tag)
        {
            ...
            {
                // 为子菜单指定显示名称、标识符和所属的菜单组，别名设为 null
        ...
        /// <param name="macroId">按钮的命令宏的 Id</param>
        ...
        {
            // 创建一个工具栏按钮对象，指定其命令宏 Id、显示名称、所属的工具栏和位置
